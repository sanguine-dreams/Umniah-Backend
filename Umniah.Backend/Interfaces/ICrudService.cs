...
public interface ICrudService<TInput, TOutput>
{
    ...
    Task<ServiceResponse<TOutput>> GetByName(string name)
        {
            return Task.FromException<ServiceResponse<TOutput>>(
                new NotSupportedException("GetByName is not supported by this service."));
        }
}